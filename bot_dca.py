...
Версия 5.31.4 (28.07.2026)
ИСПРАВЛЕНИЯ v5.31.4:
- ИСПРАВЛЕНА ЛОГИКА ОЖИДАНИЯ ИСПОЛНЕНИЯ ЛИМИТНОГО ОРДЕРА.
- Теперь бот ждет исполнения ордера до 1 часа, а не 30 секунд.
- Добавлена логика повторной попытки покупки, если ордер не исполнился за час.
- Улучшено ожидание зачисления баланса после покупки.
- Исправлена ошибка, из-за которой бот мог купить два ордера при одном запросе.
"""
import os
import sys
import asyncio
...
class BybitClient:
    ...
    async def wait_for_order_filled(self, symbol: str, order_id: str,
        ...
        """Ожидает заполнения ордера до 1 часа, проверяя статус каждые 5 секунд."""
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                # Проверяем, есть ли ордер в открытых
                open_orders = await self.get_open_orders(symbol)
                is_open = any(o.get('orderId') == order_id for o in open_orders)
                if not is_open:
                    # Если ордер не в открытых, проверяем историю
                    ...
                    for o in history:
                        if o.get('orderId') == order_id:
                            if o.get('orderStatus') == 'Filled':
                                ...
                                return False
                    # Если не нашли в истории, возможно, еще не записалось, ждем дальше
    ...
    async def wait_for_balance_credit(self, coin: str, expected_quantity: float,
        ...
        """Ожидает зачисления монет на баланс с более надежной проверкой."""
        ...
        while time.time() - start_time < timeout:
            ...
            if balance and 'equity' in balance:
                ...
                # Если баланс достиг целевого значения
                ...
                # Если баланс стабилен несколько раз, проверяем историю
...
class DCAStrategy:
    ...
    async def execute_scheduled_purchase(self, symbol: str, profit_percent: float, bot) -> Dict:
        """Выполняет запланированную покупку с долгим ожиданием и повторной попыткой."""
        ...
        # Пытаемся разместить ордер с повторной попыткой, если он не исполнился
        max_retries = 2
        retry_count = 0
        result = None
        order_placed = False
        ...
        while retry_count < max_retries and not order_placed:
            ...
            if order_filled:
                ...
                if retry_count < max_retries:
                    # Обновляем текущую цену для новой попытки
                    current_price = await self.bybit.get_symbol_price(symbol)
                    if not current_price:
...
class FastDCABot:
    ...
    async def add_executed_order_to_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str):
        ...
        if purchase_id:
            ...
            # Рекомендация по продаже больше не отправляется автоматически
